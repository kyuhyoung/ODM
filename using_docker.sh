...
#dir_data=/mnt/hdd_16tb/dataset_stereo/dabeeo/from_toan/
#dir_data=/mnt/hdd_16tb/dataset_stereo/
#dir_data=/mnt/hdd_16tb/datas/Samseong/
dir_data=/mnt/hdd_16tb/datas/
#dir_data=/home/kevin-sosa/Downloads/test_block_07044/
#: << 'END'
##########################################################################################
#   docker build
docker build --force-rm --shm-size=64g -t ${docker_name} -f Dockerfile_${docker_name} .
#END

#: << 'END'
#   docker info.
#docker run --rm -it -w $PWD -v $PWD:$PWD ${docker_name} bash docker_file/extract_docker_info.sh
docker run --privileged --rm -it -w $PWD -v $PWD:$PWD ${docker_name} sh -c ". ~/.bashrc && . ./extract_docker_info.sh"
#END
